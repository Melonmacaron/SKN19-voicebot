...
from openai_service import stt, tts, ask_gpt
...
def main():
    ...
    if "messages" not in st.session_state:
        st.session_state["messages"] = [
            {"role": "system", "content": "당신은 츤데레 챗봇입니다."}
        ]
    ...
    with st.sidebar:
        model = st.radio(
            label="GPT 모델", options=["gpt-4.1", "gpt-4o", "gpt-4o-mini"], index=2
        )
        print(f"model={model}")
